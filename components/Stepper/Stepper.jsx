...
export default function Stepper({
    ...
    return (<div className="outer-container" {...rest} onClick={() => {
        ...
        <div className={`step-circle-container ${stepCircleContainerClassName}`}
                ...
                {stepsArray.map((_, index) => {
                    ...
                    return (<React.Fragment key={stepNumber}>
                        {renderStepIndicator ? (renderStepIndicator({
                            step: stepNumber, currentStep, onStepClick: clicked => {
            ...
            {!isCompleted && (<div className={`footer-container ${footerClassName}`}>
...
function StepIndicator({step, currentStep, onClickStep, disableStepIndicators}) {
    ...
    return (<motion.div onClick={handleClick} className="step-indicator"
                        style={disableStepIndicators ? {pointerEvents: 'none', opacity: 0.5} : {}} animate={status}
                        initial={false}>
        <motion.div
            variants={{
        ...
        >
            {status === 'complete' ? (<CheckIcon className="check-icon"/>) : status === 'active' ? (
                <div className="active-dot"/>) : (<span className="step-number">{step}</span>)}
        </motion.div>
    </motion.div>);
}
