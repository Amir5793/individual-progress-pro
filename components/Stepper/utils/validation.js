    ...
    for (const field of fields) {
        if (value[field] !== undefined && value[field] !== null) {
            ...
            if (value[field].length > 200) {
                return {valid: false, error: `Reason '${field}' cannot exceed 200 characters.`};
            }
        }
    }
    return {valid: true};
...
export const validateActions = (value) => {
    if (!Array.isArray(value) || value.length === 0) {
        return {valid: false, error: 'At least one action is required to build a plan.'};
    }
 ...
 */
export const validateGoalData = (data, mode = 'goal') => {
    const errors = {};
    let isValid = true;
 ...
 */
export const validateStepInput = (currentStep, value, mode = 'goal', nextStep) => {
    if (mode === 'goal') {
        return validateGoalStep(currentStep, value);
    } else if (mode === 'habit') {
...
const validateGoalStep = (currentStep, value) => {
    switch (currentStep) {
        case 1:
            return {valid: true};
        case 2:
